...
export function useQuizScoring() {
  function calculateQuestionScore(question: Question, answer: string | string[]): number {
    if (question.type === 'multiple-choice') {
      ...
      return ansStr.trim().toLowerCase() === (question.correctAnswers[0] || '').trim().toLowerCase()
        ? 1
        : 0
    }
    return 0
  ...
  function getPartialExplanation(question: Question, answer: string | string[]): string {
  ...
  function getScoreClass(score: number, question: Question): string {
    if (question.type === 'multiple-answer' && score > 0 && score < 1)
