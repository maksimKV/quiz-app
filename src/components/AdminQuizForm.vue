<template>
  <form @submit.prevent="onSubmit" class="space-y-4 bg-white dark:bg-gray-800 p-6 rounded shadow">
    ...
    <div>
      ...
      <input v-model="form.title" type="text" class="input" :class="{'border-red-500': $v.form.title.$error}" required :disabled="loading" />
      <div v-if="$v.form.title.$error" class="text-red-500 text-xs mt-1">Title is required.</div>
    </div>
    <div>
      <label class="block font-semibold mb-1">Description</label>
      <textarea v-model="form.description" class="input" :class="{'border-red-500': $v.form.description.$error}" required :disabled="loading" />
