  ...
  <div class="max-w-md mx-auto p-8 bg-white/90 dark:bg-gray-800/90 rounded-2xl shadow-xl mt-16 border border-gray-200 dark:border-gray-700">
    ...
    <form @submit.prevent="onLogin" class="space-y-6">
      <div>
        ...
        <input v-model="email" type="email" class="input focus:ring-2 focus:ring-blue-400" required autocomplete="username" />
      </div>
      <div>
        <label class="block font-semibold mb-1 text-gray-700 dark:text-gray-200">Password</label>
        <input v-model="password" type="password" class="input focus:ring-2 focus:ring-blue-400" required autocomplete="current-password" />
      </div>
      ...
      <button type="submit" class="w-full px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 transition disabled:opacity-50 disabled:cursor-not-allowed" :disabled="loading">Login</button>
    </form>
  </div>
</template>
