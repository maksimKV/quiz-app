<template>
  <div class="max-w-2xl mx-auto p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl mt-12 border border-gray-200 dark:border-gray-700">
    <h1 class="text-3xl font-extrabold mb-8 text-center text-yellow-700 dark:text-yellow-300">Leaderboard</h1>
    <ol class="list-decimal pl-8 space-y-2">
      <li v-for="(entry, idx) in leaderboard" :key="entry.userId" class="bg-gray-50 dark:bg-gray-900 rounded-lg shadow p-4 flex items-center justify-between border border-gray-200 dark:border-gray-700">
        <div>
          <span class="font-semibold text-lg">{{ userInfo(entry.userId)?.name || entry.userId }}</span>
